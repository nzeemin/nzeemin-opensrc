...
void SDLStub::processEvents() {
	...
	while(SDL_PollEvent(&ev)) {
		...
		case SDL_KEYDOWN:
			...
			if (keystate[SDLK_ESCAPE] && keystate[SDLK_RETURN])  // SELECT and START buttons
				_pi.quit = true;
			_pi.button = (keystate[SDLK_SPACE] || keystate[SDLK_LCTRL]);  // X or A buttons

			_pi.dirMask = 0;
			if (keystate[SDLK_RIGHT]) _pi.dirMask |= PlayerInput::DIR_RIGHT;
			if (keystate[SDLK_LEFT])  _pi.dirMask |= PlayerInput::DIR_LEFT;
			if (keystate[SDLK_UP])    _pi.dirMask |= PlayerInput::DIR_UP;
			if (keystate[SDLK_DOWN])  _pi.dirMask |= PlayerInput::DIR_DOWN;

			if (ev.type == SDL_KEYDOWN)
			{
				switch(ev.key.keysym.sym) {
				case SDLK_ESCAPE:  // SELECT button on RZX50
					_pi.code = true;
					break;
				case SDLK_RETURN:  // START button on RZX50
					_pi.pause = true;
					break;
				case SDLK_TAB:  // LSHIFT button
					volume -= 5;
					if (volume < 0) volume = 0;
					break;
				case SDLK_BACKSPACE:  // RSHIFT button
					volume += 5;
...
void SDLStub::prepareGfxMode() {
	int w = DSCREEN_W;
	int h = DSCREEN_H;
	_screen = SDL_SetVideoMode(w, h, 16, /*_fullscreen ? (SDL_FULLSCREEN | SDL_HWSURFACE) : SDL_HWSURFACE*/ 0);
	if (!_screen) {
...
void SDLStub::point1x(uint16 *dst, uint16 dstPitch, const uint16 *src, uint16 srcPitch, uint16 w, uint16 h) {
	dstPitch >>= 1;
	uint16 gapw = (DSCREEN_W - SCREEN_W) / 2;
	uint16 gaph = (DSCREEN_H - SCREEN_H) / 2;
	dst += dstPitch * gaph;
	while (h--) {
		memcpy(dst + gapw, src, w * 2);
		dst += dstPitch;
		src += srcPitch;
	}
}
